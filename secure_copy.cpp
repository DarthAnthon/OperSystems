...
void lock_mtx(pthread_mutex_t *m, int tid) {
  timespec ts;
  while (pthread_mutex_timedlock(
      m, (clock_gettime(CLOCK_REALTIME, &ts), ts.tv_sec += 5, &ts))) {
...
bool proc_file(const std::string &in, const std::string &outdir) {
  ...
  f.close();
  if (sz)
    caesar(buf.data(), buf.data(), sz);
  std::string out = outdir + "/" +
                    (in.find_last_of("/\\") == std::string::npos
                         ? in
...
void *worker(void *arg) {
  ...
  while (true) {
    lock_mtx(&sh->mtx, tid);
    while (sh->files.empty() && !sh->stop)
      pthread_cond_wait(&sh->cond, &sh->mtx);
    if (sh->files.empty() && sh->stop) {
      pthread_mutex_unlock(&sh->mtx);
      break;
    }
    std::string f = sh->files.front();
    sh->files.pop();
    pthread_mutex_unlock(&sh->mtx);
    ...
    lock_mtx(&sh->mtx, tid);
    sh->log << "[" << tbuf << "] [thread " << tid << "] " << f << " "
            << (ok ? "SUCCESS" : "ERROR") << " " << dt << "s\n";
    if (ok) {
      sh->done++;
      sh->times.push_back(dt);
    }
    pthread_mutex_unlock(&sh->mtx);
  }
  return nullptr;
...
Stats sequential(const std::vector<std::string> &files, const std::string &outdir,
                 std::ofstream &log) {
  double t0 = now_sec();
  int okcnt = 0;
  double total = 0;
  for (const auto &f : files) {
    ...
    time_t now = time(nullptr);
    strftime(tbuf, 20, "%Y-%m-%d %H:%M:%S", localtime(&now));
    log << "[" << tbuf << "] [sequential] " << f << " " << (ok ? "SUCCESS" : "ERROR")
        << " " << ft << "s\n";
    if (ok) {
      okcnt++;
    }
...
Stats parallel(const std::vector<std::string> &files, const std::string &outdir,
               std::ofstream &log) {
  Shared sh;
  sh.outdir = outdir;
  sh.log.swap(log);
  ...
  double t0 = now_sec();
  pthread_mutex_lock(&sh.mtx); // захват мьютекса, для получения доступа к списку файлов и
                               // флагу завершения
  while (!sh.files.empty()) {
    ...
    pthread_mutex_lock(&sh.mtx); // очередной захват мьютекса для получения главным
                                 // потоком доступа к списку файлов и флагу
  }
...
int main(int argc, char **argv) {
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0]
              << " [--mode=sequential|parallel] files... outdir key\n";
    return 1;
  }
  int fstart = 1;
  ...
  if (!log)
    return 1;
  if (mode == AUTO)
    mode = (files.size() < 5) ? SEQ : PAR;
  Stats res =
      (mode == SEQ) ? sequential(files, outdir, log) : parallel(files, outdir, log);
